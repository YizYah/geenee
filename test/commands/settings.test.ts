...
describe('settings', () => {
  test
  .stdout()
  .command(['settings', 'sampleCode'])
  .it('runs settings with all args and flags', ctx => {
    expect(ctx.stdout).to.contain('You have executed the settings command')
  })

  test
  .stderr()
  .command(['settings', 'sampleBadArgValue'])
  .catch(error => {
